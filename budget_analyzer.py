...
def main():
    ...
    # Example: Load data if JSON exists, otherwise use template
    try:
        analyzer.load_from_json('budget_data.json')
    except FileNotFoundError:
        print("No budget_data.json found. Please fill out BUDGET_AUDIT_TEMPLATE.md first,")
        print("then create budget_data.json based on the template structure.")
        return
    ...
    # Save report to file
    with open('budget_analysis_report.txt', 'w') as f:
        f.write(report)
    print("\nReport saved to budget_analysis_report.txt")
